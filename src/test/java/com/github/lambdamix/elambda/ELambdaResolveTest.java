...
public class ELambdaResolveTest extends BasePlatformTestCase {
    ...
    public void testLocalPattern() {
        myFixture.configureByFile(getFileName());
        PsiElement element = myFixture.getFile().findElementAt(myFixture.getCaretOffset());
        LocalPattern pattern = PsiTreeUtil.getParentOfType(element, LocalPattern.class);
        assertNotNull(pattern);
        LocalPatternReference reference = pattern.getReference();
        DataConstructor dataConstructor = reference.resolve();
        assertNotNull(dataConstructor);
    ...
    public void testMod() {
        ...
        PsiElement element = myFixture.getFile().findElementAt(myFixture.getCaretOffset());
        Mod mod = PsiTreeUtil.getParentOfType(element, Mod.class);
        assertNotNull(mod);
        ModReference reference = mod.getReference();
        ELambdaFile eLambdaFile = reference.resolve();
        assertNotNull(eLambdaFile);
        assertEquals(getFileNameAux(), eLambdaFile.getName());
    }
    ...
    public void testRemotePattern() {
        myFixture.configureByFiles(getFileName(), getFileNameAux());
        PsiElement element = myFixture.getFile().findElementAt(myFixture.getCaretOffset());
        RemotePattern remotePattern = PsiTreeUtil.getParentOfType(element, RemotePattern.class);
        assertNotNull(remotePattern);
        RemotePatternReference reference = remotePattern.getReference();
        DataConstructor dataConstructor = reference.resolve();
        assertNotNull(dataConstructor);
        assertEquals(remotePattern.getName(), dataConstructor.getName());
        assertEquals(getFileNameAux(), dataConstructor.getContainingFile().getName());
    }
